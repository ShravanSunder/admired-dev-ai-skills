# discuss-clarify-mental-models

- **ai-tools path:** `/Users/shravansunder/dev/ai-tools/plugins/shravan-dev-workflow/skills/discuss-clarify-mental-models/`
- **Plugin:** shravan-dev-workflow
- **Last reviewed:** 2026-07-09

## Admired sources

| Upstream repo (submodule) | Path inside submodule | What we borrowed | Do not copy |
|---------------------------|----------------------|------------------|-------------|
| cursor-plugins | `pstack/skills/interrogate/`, decision/action trail patterns | material interjections, challenge questions, session requirements | Cursor-specific commands, one ritual forcing every question |
| mattpocock-skills | grilling / domain modeling | discussion and domain-model challenge patterns | discussion as substitute for source-backed specs |
| addyosmani-agent-skills | interview / idea / doubt patterns | lifecycle taxonomy for reconvergence | wholesale Addy workflow |
| steipete-agent-scripts | owner-brief / maintainer patterns | owner-facing clarification briefs | personal orchestration tree |
| obra-superpowers | `skills/brainstorming/` | design-before-code process inspiration | Superpowers as the discussion skill |

## Reverse links

- [cursor-plugins](../../repo-index/cursor-plugins.md)
- [mattpocock-skills](../../repo-index/mattpocock-skills.md)
- [addyosmani-agent-skills](../../repo-index/addyosmani-agent-skills.md)
- [steipete-agent-scripts](../../repo-index/steipete-agent-scripts.md)
- [obra-superpowers](../../repo-index/obra-superpowers.md)
