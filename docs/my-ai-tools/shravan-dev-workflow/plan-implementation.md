# plan-implementation

- **ai-tools path:** `/Users/shravansunder/dev/ai-tools/plugins/shravan-dev-workflow/skills/plan-implementation/`
- **Plugin:** shravan-dev-workflow
- **Last reviewed:** 2026-08-27
- **Renamed from:** `plan-creation-swarm` (retired in ai-tools)

## Admired sources

| Upstream repo (submodule) | Path inside submodule | What we borrowed | Do not copy |
|---------------------------|----------------------|------------------|-------------|
| obra-superpowers | `skills/writing-plans/` | DAG-shaped vertical slices, proof gates, subagent suitability | wholesale plan templates without repo proof |
| cursor-plugins | `pstack/skills/principle-prove-it-works/`, `principle-boundary-discipline/` | prove-it-works, boundary clarity | anti-planning stance; poteto multi-PR checklist as the local plan schema |
| cursor-plugins | `pstack/skills/poteto-mode/playbooks/multi-phase-plan.md` | verified-unit sequencing inspiration | Cursor `check-plan.mjs` / orch scripts |
| codex-curated-skills | Codex subagent / platform guidance | harness-fit for parallel lanes | treating scaffolding as planning philosophy |
| Dimillian-Skills | batch refactor orchestration | parallel bounded work lanes | Apple/UI-specific orchestration |

## Reverse links

- [obra-superpowers](../../repo-index/obra-superpowers.md)
- [cursor-plugins](../../repo-index/cursor-plugins.md)
- [codex-curated-skills](../../repo-index/codex-curated-skills.md)
- [Dimillian-Skills](../../repo-index/Dimillian-Skills.md)
